...
#include <stdint.h>
#include "Invn/EmbUtils/InvExport.h"
...
#define ALGO_INVN_CALIBRATION_GYRO_BT_CONFIG_SIZE  628
#define ALGO_INVN_ORIENTATION_CONFIG_SIZE          624
#define ALGO_INVN_PREDICTIVEQUATERNION_CONFIG_SIZE 152
...
#define ALGO_INVN_CALIBRATION_GYRO_BT_DEFAULT_SAMPLING 20000

 /* Parameters for Predictive Quaternion */
#define ALGO_INVN_PREDICTIVEQUATERNION_CONSTANT_METHOD_ENNUM 1
#define ALGO_INVN_PREDICTIVEQUATERNION_HYBRID_METHOD_ENNUM 2
#define ALGO_INVN_PREDICTIVEQUATERNION_DEFAULT_METHOD_ENNUM ALGO_INVN_PREDICTIVEQUATERNION_HYBRID_METHOD_ENNUM
#define ALGO_INVN_PREDICTIVEQUATERNION_DEFAULT_PREDICTIVE_TIME_US 20000
#define ALGO_INVN_PREDICTIVEQUATERNION_SHORTER_PREDICTIVE_TIME_US  5000
...
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_ACGO_CONFIG_SIZE to use internally.
* \param[in] gyr[3] gyrometer measurment in [rps]. fxp format is \b q16
* \param[in] dt_us time since last sample in [us].
* \param[in] gyr_accuracy accuracy of current gyrometer measurement (0 to 3).
* \param[in] opt Additional parameters, can be NULL.
...
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYROBT_CONFIG_SIZE to use internally. 
* \warning This method sets default parameters value, it will resets the algorithm and set offset and accuracy to 0. The bias tracking routine will be available.
...
/**
* \brief Enables the bias tracking routine.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYRO_BT_CONFIG_SIZE to use internally.
* \warning At enable, bias tracker states will resets to last known FNM solution or last solution set by Algo_InvnCalibration_GyroBiasTrackerFxp_SetBias function.
* \warning This function will keep current algo parameters.
* \ingroup GyroBiasTracker
*/
void INV_EXPORT Algo_InvnCalibration_GyroBiasTrackerFxp_EnableTracking(void* data);

/**
* \brief Disable the bias tracking routine, FNM outputs (bias and accuracy) will be directly feed trough Bias Tracker. Algo_InvnCalibration_GyroBiasTrackerFxp_SetBias can still be used to overwrite current outputs.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYRO_BT_CONFIG_SIZE to use internally.
* \warning Using this routine will make the algo feed the FNM bias and accuracy directly through the algorithm.
* \ingroup GyroBiasTracker
*/
void INV_EXPORT Algo_InvnCalibration_GyroBiasTrackerFxp_DisableTracking(void* data);
...
* \param[in] opt Additional parameters, can be NULL.
* \retval -1 if sampling period is >20000us. Previous sampling period is kept.
* \retval 0 otherwise.
* \warning The maximum sampling period for bias tracker is 20000us. This method will keep current offset and accuracy level.
* \ingroup GyroBiasTracker
*/
int8_t INV_EXPORT Algo_InvnCalibration_GyroBiasTrackerFxp_SetGyrSamplingPeriod(void* data, uint32_t gyro_sampling_period, void *opt);

/**
* \brief This function allows to change magnetometer sampling period on the fly without reseting the algorithm and so keeping accuracy level.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYRO_BT_CONFIG_SIZE to use internally.
* \param[in] mag_sampling_period sampling period [us] of the magnetometer sensor.
* \retval -1 if sampling period is >20000us. Previous sampling period is kept.
* \retval 0 otherwise.
* \warning The maximum sampling period for bias tracker is 20000us for all sensors. This method will keep current offset and accuracy level.
* \ingroup GyroBiasTracker
*/
int8_t INV_EXPORT Algo_InvnCalibration_GyroBiasTrackerFxp_SetMagSamplingPeriod(void* data, uint32_t mag_sampling_period);

/**
* \brief This function allows to change accelerometer sampling period on the fly without reseting the algorithm and so keeping accuracy level.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYRO_BT_CONFIG_SIZE to use internally.
* \param[in] acc_sampling_period sampling period [us] of the accelerometer sensor.
* \param[in] opt Additional parameters, can be NULL.
* \retval -1 if sampling period is >20000us. Previous sampling period is kept.
* \retval 0 otherwise.
* \warning The maximum sampling period for bias tracker is 20000us for all sensors. This method will keep current offset and accuracy level.
* \ingroup GyroBiasTracker
*/
int8_t INV_EXPORT Algo_InvnCalibration_GyroBiasTrackerFxp_SetAccSamplingPeriod(void* data, uint32_t acc_sampling_period, void *opt);

/**
* \brief This function allows to set a bias with an associated accuracy level.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYROBT_CONFIG_SIZE to use internally.
* \param[in] gyro_offset gyrometer offset in [1/2000 dps], fxp format is \b q30.\n i.e \f$ 2000 dps = 2^{30} \f$
...
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYROBT_CONFIG_SIZE to use internally.
* \param[in] mag_calib calibrated magnetometer data in [uT]. fxp format is \b q16
* \param[out] mag_accuracy_flag accuracy flag of calibrated magnetometer (0 to 3).
* \ingroup GyroBiasTracker
*/
...
* \brief Updates the algorithm with new accelerometer sensor data.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYROBT_CONFIG_SIZE to use internally.
* \param[in] acc_calib calibrated accelerometer data in [g]. fxp format is \b q25.
...
* \brief Updates the algorithm with new gyrometer data.
* \param[in] data Address of the structure of size ALGO_INVN_CALIBRATION_GYROBT_CONFIG_SIZE to use internally.
* \param[in] gyr_uncalib uncalibrated gyrometer measurment in [1/2000 dps], fxp format is \b q30.\n i.e \f$ 2000 dps = 2^{30} \f$
* \param[in] gyr_offset_fnm  <i> (optional) </i> gyrometer offset computed by FNM algorithm in [1/2000 dps], fxp format is \b q30. \n If the update function is called with a \c NULL pointer, this input will be ignored.
* \param[in] gyr_accuracy_flag_fnm  <i> (optional) </i> accuracy flag (0 to 3) of FNM offset. \n If gyr_offset_fnm is a \c NULL pointer, this input will be ignored.
* \param[in] gyr_temp  <i> (optional) </i> temperature of gyrometer sensor in [\f$ ^\circ \f$ C*100]. If the update is called with a constant value of 0 this input will have no effect.
* \param[out] gyr_offset gyrometer offset in [1/2000 dps], fxp format is \b q30.\n i.e \f$ 2000 dps = 2^{30} \f$
* \return Bias tracker status
* \retval 1 when new sample has been taken into account
* \retval 2 when a new offset has been validated
* \retval 0 otherwise.
* \sa GyroCalibration
* \ingroup GyroBiasTracker
*/
...
* \param[in] data Address of the structure of size ALGO_INVN_ORIENTATION_CONFIG_SIZE bytes to use internally.
